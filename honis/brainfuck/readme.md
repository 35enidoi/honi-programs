...
入力をinputによる標準入力ではなく、speinpで入力を渡すことができます。  
brainfuckのコードでspeinpで与えられた文字列以上の文字をとろうとした場合、一回目はEOFを表す26が返されます。  
2回目はSpeinpTooGetErrorが送出されます。
...
出力をreturnするようになります。  
debugやstepmodeの標準出力は無効になり、printされません。  
> [!WARNING]  
> yiemodeとの併用は不可能です。  
> もしyiemodeもTrueの場合、retmodeが優先されます。  
...
出力の情報を持ったlistを返します。  
retmodeと同様、標準出力は無効になります。  
stepmodeがTrueの時、listの情報が変わります。  
